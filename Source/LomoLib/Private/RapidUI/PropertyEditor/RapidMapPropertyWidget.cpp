    ...
    for (int32 SparseIndex = 0; SparseIndex < MapHelper.GetMaxIndex(); ++SparseIndex)
    {
        if (MapHelper.IsValidIndex(SparseIndex))
        {
            if (RealIndex == PairIndex)
            {
                ...
                DeleteDelegate.BindUFunction(this, "HandleElementDeleteClicked", PairIndex);
                ...
                URapidPropertyWidget* KeyWidget = nullptr;
                
                // 根据键属性类型创建控件
                if (KeyProperty->IsA<FFloatProperty>() && PropertyEditor->GetFloatPropertyWidgetClass())
                {
                    KeyWidget = CreateWidget<URapidPropertyWidget>(this, PropertyEditor->GetFloatPropertyWidgetClass());
                }
                else if (KeyProperty->IsA<FIntProperty>() && PropertyEditor->GetIntPropertyWidgetClass())
                {
                    KeyWidget = CreateWidget<URapidPropertyWidget>(this, PropertyEditor->GetIntPropertyWidgetClass());
                }
                else if (KeyProperty->IsA<FBoolProperty>() && PropertyEditor->GetBoolPropertyWidgetClass())
                {
                    KeyWidget = CreateWidget<URapidPropertyWidget>(this, PropertyEditor->GetBoolPropertyWidgetClass());
                }
                else if ((KeyProperty->IsA<FStrProperty>() || KeyProperty->IsA<FNameProperty>() || KeyProperty->IsA<FTextProperty>()) && PropertyEditor->GetStringPropertyWidgetClass())
                {
                    KeyWidget = CreateWidget<URapidPropertyWidget>(this, PropertyEditor->GetStringPropertyWidgetClass());
                }
                ...
                URapidPropertyWidget* ValueWidget = nullptr;
                
                // 根据值属性类型创建控件
                if (ValueProperty->IsA<FFloatProperty>() && PropertyEditor->GetFloatPropertyWidgetClass())
                {
                    ValueWidget = CreateWidget<URapidPropertyWidget>(this, PropertyEditor->GetFloatPropertyWidgetClass());
                }
                else if (ValueProperty->IsA<FIntProperty>() && PropertyEditor->GetIntPropertyWidgetClass())
                {
                    ValueWidget = CreateWidget<URapidPropertyWidget>(this, PropertyEditor->GetIntPropertyWidgetClass());
                }
                else if (ValueProperty->IsA<FBoolProperty>() && PropertyEditor->GetBoolPropertyWidgetClass())
                {
                    ValueWidget = CreateWidget<URapidPropertyWidget>(this, PropertyEditor->GetBoolPropertyWidgetClass());
                }
                else if ((ValueProperty->IsA<FStrProperty>() || ValueProperty->IsA<FNameProperty>() || ValueProperty->IsA<FTextProperty>()) && PropertyEditor->GetStringPropertyWidgetClass())
                {
                    ValueWidget = CreateWidget<URapidPropertyWidget>(this, PropertyEditor->GetStringPropertyWidgetClass());
                }
                else if (ValueProperty->IsA<FStructProperty>() && PropertyEditor->GetStructPropertyWidgetClass())
                {
                    ValueWidget = CreateWidget<URapidPropertyWidget>(this, PropertyEditor->GetStructPropertyWidgetClass());
                }
                else if (ValueProperty->IsA<FArrayProperty>() && PropertyEditor->GetArrayPropertyWidgetClass())
                {
                    ValueWidget = CreateWidget<URapidPropertyWidget>(this, PropertyEditor->GetArrayPropertyWidgetClass());
                }
                else if (ValueProperty->IsA<FMapProperty>() && PropertyEditor->GetMapPropertyWidgetClass())
                {
                    ValueWidget = CreateWidget<URapidPropertyWidget>(this, PropertyEditor->GetMapPropertyWidgetClass());
                }
